...
async function getConfig() {
  if (!headers['Cookie']) {
    ...
    headers['Cookie'] = respHeaders['Set-Cookie']?.split(';')[0] || respHeaders['set-cookie'][0].split(';')[0];
  }
...
async function extractCards(url) {
  ...
  $('.movie-card').each((_, each) => {
    ...
    cards.push({
      vod_id: path,
      vod_name: $(each).find('.card-info > h4').text(),
      vod_pic: $(each).find('.card-img img').attr('data-src'),
      vod_remarks: $(each).find('.episode-badge').text(),
      vod_pubdate: $(each).find('.card-meta').text(),
      ext: {
