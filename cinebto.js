/**
 * XPTV Extension — Cineb.to
 * https://cinebto.com
 * * 架构说明：
 * 1. 列表与搜索：直连高效 JSON 数据库，秒出封面与数据。
 * 2. 播放引擎：使用 Vidking + Vidlink 双重无加密源，配合 parse:1 唤醒 XPTV 内置嗅探器直接抓取直链。
 */
...
const appConfig = { ver:1, title:'Cineb', site:BASE, tabs:TABS }
...
async function getPlayinfo(ext) {
    ...
    if(!tmdbId){
        $utils.toastError('缺少 TMDB ID，无法播放')
        return jsonify({urls:[''],headers:[{}]})
    }

    $utils.toastInfo('启动内核嗅探中...请稍候')

    let playUrls = []
    
    // 构造双重备用嗅探源，XPTV会自动尝试提取视频
    if (kind === 'tv' || kind === MEDIA.TV) {
        playUrls.push(`https://www.vidking.net/embed/tv/${tmdbId}/${seasonId}/${epId}`)
        playUrls.push(`https://vidlink.pro/tv/${tmdbId}/${seasonId}/${epId}`)
    } else {
        playUrls.push(`https://www.vidking.net/embed/movie/${tmdbId}`)
        playUrls.push(`https://vidlink.pro/movie/${tmdbId}`)
    }

    // 指令解析：parse: 1 代表命令 XPTV 打开自带的浏览器执行网页嗅探
    return jsonify({
        urls: playUrls,
        parse: 1, 
